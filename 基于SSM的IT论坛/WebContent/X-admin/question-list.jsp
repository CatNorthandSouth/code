...
<html>
    ...
    <body>
        ...
        <div class="x-body">
            ...
            <xblock><button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon">&#xe640;</i>批量删除</button><button class="layui-btn" onclick="question_add('添加问题','question-add.jsp','600','500')"><i class="layui-icon">&#xe608;</i>添加</button><span class="x-right" style="line-height:40px">共有数据：88 条</span></xblock>
            <table class="layui-table">
                ...
                <tbody id="post-tbody-list">
                    <tr>
                        ...
                        <td class="td-manage">
                            <a title="编辑" href="javascript:;" onclick="question_edit('编辑','question-edit.jsp','4','','510')"
        ...
        <script>
                  ...
                  function delAll() {
                    if (checkboxlist.length == 0) {
     				 ...
     				 layer.confirm('确认要删除吗？',function(index){
     	                    //捉到所有被选中的，发异步进行删除
     	                    $.ajax({
     	                    	url:"${pageContext.request.contextPath}/HdeletePostList",
     	                    	dataType:"JSON",
     	                    	type:"post",
     	                    	data:{
    		...
    		function findlistpost(sum,str) {
    			...
    			init1("HfindPost",str);
    	   	       $.ajax({
       	        	url:"${pageContext.request.contextPath}/HfindPost",
       	        	dataType:"JSON",
       	        	data:{
       	        		postId:str
       	        	},
       	        	...
       	        	success:function(data){
       	        		...
       	        		for (var i = pagecount; i < pagecount1; i++) {
    						...
    						((data.list)[0])[i].postTitle+'</a></u></td><td >'+
       						...
       						+'<a title="编辑" href="javascript:;" onclick="question_edit(\'编辑\',\'question-edit.jsp\',\'4\',\'\',\'510\')" class="ml-5" style="text-decoration:none">'
       						+' <i class="layui-icon">&#xe642;</i>'
    	...
    	function initList(sum) {
    	       ...
    	       $.ajax({
    	        	url:"${pageContext.request.contextPath}/HselectPostList",
    	        	dataType:"JSON",
    	        	data:{},
    	        	type:"post",
    	        	success:function(data){
    	        		...
    	        		var str='';
    	        		var str1='';
    	        		str1=str1+'<tr><th><input type="checkbox"  name="" ></th> <th>ID</th><th> 标题</th><th>分类</th><th>来源</th><th>更新时间</th><th>浏览次数</th><th>操作</th></tr>';
    	        		for (var i = pagecount; i < pagecount1; i++) {
    						str=str+'<tr><td><input type="checkbox"  class="checktt"  value="'+((data.list)[0])[i].postId+'" name=""></td><td>'+
    						((data.list)[0])[i].postId+'</td><td><u style="cursor:pointer" onclick="question_show()"><a target="_blank" href="${pageContext.request.contextPath}/togoPostDetails?id='+((data.list)[0])[i].postId+'">'+
    						((data.list)[0])[i].postTitle+'</a></u></td><td >'+
    						((data.list)[1])[i]+'</td><td >'+
    						((data.list)[0])[i].postAdmin+'</td><td >'+
    						timestampToTime(((data.list)[0])[i].postUpdateTime)+'</td><td >'+
    						((data.list)[0])[i].postReward+'</td><td class="td-manage">'
    						+'<a title="编辑" href="javascript:;" onclick="question_edit(\'编辑\',\'question-edit.jsp?id='+((data.list)[0])[i].postId+'\',\'4\',\'\',\'510\')" class="ml-5" style="text-decoration:none">'
    						+' <i class="layui-icon">&#xe642;</i>'
    						+'</a><a title="删除" href="javascript:;" onclick="question_del(this,'+((data.list)[0])[i].postId+')" style="text-decoration:none"> '
    						+'<i class="layui-icon">&#xe640;</i></a></td></tr>';
