...
<html>
    ...
    <body>
        ...
        <div class="x-body">
            ...
            <xblock><button id="admin-delAll" class="layui-btn layui-btn-danger" ><i class="layui-icon">&#xe640;</i>批量删除</button><span class="x-right" id="post-sum" style="line-height:40px">共有数据：88 条</span></xblock>
       	        	...
       	        	success:function(data){
       	        		...
       	        		for (var i = pagecount; i < pagecount1; i++) {
    						...
    						((data.list)[0])[i].postTitle+'</a></u></td><td >'+
       						...
       						((data.list)[0])[i].postReward+'</td><td class="td-manage" id="td-manage">';
    		        		if ($("#hiddenvalue").val() != "true") {	
    		            		$("#td-manage").find("a").attr("onclick","warnFunction()");
    							...
    							str=str+'<a title="编辑" href="javascript:;" onclick="question_edit(\'编辑\',\'question-edit.jsp?id='+((data.list)[0])[i].postId+'\',\'4\',\'\',\'510\')" class="ml-5" style="text-decoration:none">'
        						+' <i class="layui-icon">&#xe642;</i>'
    	...
    	function initList(sum) {
    	       ...
    	       $.ajax({
    	        	...
    	        	success:function(data){
    	        		...
    	        		$("#post-sum").text("共有数据："+data.pageCount+" 条");
    	        		var str='';
    	        		var str1='';
    	        		str1=str1+'<tr><th><input type="checkbox" lay-filter="checkbox_v1" name="" ></th> <th>ID</th><th> 标题</th><th>分类</th><th>来源</th><th>更新时间</th><th>浏览次数</th><th>操作</th></tr>';
    	        		for (var i = pagecount; i < pagecount1; i++) {
    		        		...
    		        		if ($("#hiddenvalue").val() != "true") {	
    							...
    							str=str+'<a title="编辑" href="javascript:;" onclick="question_edit(\'编辑\',\'X-admin/question-edit.jsp?id='+((data.list)[0])[i].postId+'\',\'4\',\'\',\'510\')" class="ml-5" style="text-decoration:none">'
        						+' <i class="layui-icon">&#xe642;</i>'
        						+'</a><a title="删除" href="javascript:;" onclick="question_del(this,'+((data.list)[0])[i].postId+')" style="text-decoration:none"> '
        						+'<i class="layui-icon">&#xe640;</i></a>';  							
    						}
