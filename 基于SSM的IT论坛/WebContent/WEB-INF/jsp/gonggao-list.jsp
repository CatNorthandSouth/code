...
<html>
    ...
    <body>
        ...
        <div class="x-body">
            ...
            <xblock><button id="admin-delAll"  class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon">&#xe640;</i>批量删除</button><button class="layui-btn" id="admin-addUser" onclick="question_add('添加问题','X-admin/gonggao-add.jsp','600','500')"><i class="layui-icon">&#xe608;</i>添加</button><span class="x-right" id="gonggao-sum" style="line-height:40px">共有数据：88 条</span></xblock>
            <table class="layui-table">
                ...
                <tbody id="post-tbody-list">
                    <tr>
                        ...
                        <td class="td-manage">
                            <a title="编辑" href="javascript:;" onclick="question_edit('编辑','X-admin/gonggao-edit.jsp','4','','510')"
       	        		...
       	        		for (var i = pagecount; i < pagecount1; i++) {
		            		...
		            		if ($("#hiddenvalue").val() != "true") {
        						...
        						+'<i class="layui-icon">&#xe640;</i></a>';
    					      	...
    					      	str=str+'<a title="编辑" href="javascript:;" onclick="question_edit(\'编辑\',\'X-admin/gonggao-edit.jsp?id='+((data.list)[0])[i].postId+'\',\'4\',\'\',\'510\')" class="ml-5" style="text-decoration:none">'
        						+' <i class="layui-icon">&#xe642;</i>'
            ...
            function question_del(obj,id){
                layer.confirm('确认要删除吗？',function(index){
                   ...
                   // transportId("HdeleteGongGao",id);
        			$.ajax({
        				url:"${pageContext.request.contextPath}/HdeleteGongGao",
        				data:{
        					postId:id
        				},
        				dataType:"JSON",
    	...
    	function initList(sum) {
    	       ...
    	       $.ajax({
    	        	...
    	        	success:function(data){
    	        		...
    	        		$("#gonggao-sum").text("共有数据："+data.pageCount+"条");
    	        		var str='';
    	        		var str1='';
    	        		str1=str1+'<tr><th><input type="checkbox" lay-filter="checkbox_v1" name="" ></th> <th>ID</th><th> 标题</th><th>分类</th><th>来源</th><th>更新时间</th><th>浏览次数</th><th>操作</th></tr>';
    	        		for (var i = pagecount; i < pagecount1; i++) {
