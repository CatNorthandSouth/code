...
<!-- 					<li><span class="redcolor">全部</span></li>
		...
		<div class="down-file-div" id="down-file-div">
			<form action="${pageContext.request.contextPath}/FileMYUpload" method="post" accept-charset="UTF-8" enctype="multipart/form-data">
		...
		<div class="down-content">
			<div class="down-content-title" id="down-content-title">
				<span class="new-upload" id="new-upload" name="0">我的上传</span>
				<span class="more-load" id="more-load" name="1">下载记录</span>
			</div>
...
<script type="text/javascript">
	...
	function initFileList(sum){
		$.ajax({
			...
			success:function(data){
				...
				if (data.length == 0) {
			 		str=str+'<img id="kongkongid" alt="" src="img/kongkong.jpg" style="width: 800px;height: 400px;margin-left: 12%;margin-top: 20px;">';
				} 
				...
				for (var i = 0; i < sum; i++) {
					...
					+'</em></label></div></dd>';
					if (typeSort == 1) {
						str=str+'<label class="click-delete1" id="'+data[i].downId+'">立即删除</label></dl>'
					} else {
						if ($("#hiddenUserId").val() == data[i].fileUpId) {
							str=str+'<label class="down-click" id="'+data[i].fileId+'">立即下载</label><label class="click-delete" id="'+data[i].fileId+'">立即删除</label></dl>';
						} else {
							str=str+'</dl>';
						}
					}
	...
	$(document).ready(function () {
		...
		/* 	var htmlHeight=document.body.scrollHeight||document.documentElement.scrollHeight;
			...
			if (winH + top > textH - 30) {
				...
				typeId=$("#downtypelist").find("li.redcolor").val();
				//alert(typeId);
				initFileList(sum);
	...
	$("#down-content-list").on("click",".click-delete1",function(){
		$.ajax({
			url:"deleteMyFileHistory",
			data:{
				fileid:$(this).attr("id"),
	...
	$("#down-content-list").on("click",".click-delete",function(){
		$.ajax({
			url:"deleteMyFile",
			data:{
				fileid:$(this).attr("id"),
