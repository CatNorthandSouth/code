...
<!--             <form class="layui-form x-center" action="" style="width:80%">
            ...
            <xblock><button id="admin-delAll" class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon">&#xe640;</i>批量删除</button><span class="x-right" style="line-height:40px">共有数据：${Logsum} 条</span></xblock>
        ...
        <script>
             ...
             function delAll() {
            	  if (checkboxlist.length == 0) {
   				 ...
   				 layer.confirm('确认要删除吗？',function(index){
   	                    //捉到所有被选中的，发异步进行删除
   	                    $.ajax({
   	                    	url:"${pageContext.request.contextPath}/HdeleteListLog",
   	                    	dataType:"JSON",
   	                    	type:"post",
   	                    	data:{
        	...
        	function initList(sum) {
        		 $.ajax({
     	        	url:"${pageContext.request.contextPath}/selectSysLogAll",
     	        	dataType:"JSON",
     	        	data:{},
     	        	type:"post",
     	        	success:function(data){
     	        		...
     	        		str1=str1+'<tr><th><input type="checkbox" lay-filter="checkbox_v1" name="'+pagecount1+'" ></th><th>ID</th><th>类型</th><th>内容</th><th>用户名</th><th>客户端IP</th><th>时间</th><th>操作</th></tr> ';
     	        		for (var i = pagecount; i < pagecount1; i++) {
     	        			str=str+'<tr><td><input type="checkbox" value="'+((data.list))[i].id+'" name="">';
     	        			str=str+'</td><td>'+((data.list))[i].id+'</td><td>';
     	        			str=str+'<u style="cursor:pointer" onclick="member_show(\'张三\',\'member-show.jsp\',\'10001\',\'360\',\'400\')">';
     	        			str=str+((data.list))[i].type+'</u></td><td >';  
     	        			str=str+((data.list))[i].content+'</td><td >';
     	        			str=str+((data.list))[i].username+'</td><td >';
     	        			str=str+((data.list))[i].ip;
     	        			str=str+'</td><td>'+timestampToTime(((data.list))[i].logintime);
     	        			str=str+'</td>';
     	                	...
     	                	if ($("#hiddenvalue").val() != "true") {
         	        			...
         	        			str=str+'<a title="删除" href="javascript:;" onclick="admin_del(this,\''+((data.list))[i].id+'\')" ';
         	        			str=str+'style="text-decoration:none"><i class="layui-icon">&#xe640;</i></a>';
     	    				}
