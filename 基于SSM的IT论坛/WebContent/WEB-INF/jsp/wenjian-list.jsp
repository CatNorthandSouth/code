...
<html>
    ...
    <body>
        ...
        <div class="x-body">
            ...
            <xblock><button id="admin-delAll" class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon">&#xe640;</i>批量删除</button><span class="x-right" id="wenjian-sum" style="line-height:40px">共有数据：88 条</span></xblock>
            <table class="layui-table">
                ...
                <tbody id="post-tbody-list">
                    <tr>
                        ...
                        <td class="td-manage">
                            <a title="编辑" href="javascript:;" onclick="question_edit('编辑','wenjian-edit.jsp','4','','510')"
        ...
        <script>
           ...
           /*        form.on('checkbox(checkbox_v1)',function(data){
                      ...
                      if (checkboxlist.length == 0) {
                      	   layer.msg('请选择您要删除的信息', {icon: 1});
          				 	...
          				 	layer.confirm('确认要删除吗？',function(index){
          	                    //捉到所有被选中的，发异步进行删除
          	                    $.ajax({
          	                    	url:"${pageContext.request.contextPath}/HdeleteFileList",
          	                    	dataType:"JSON",
          	                    	type:"post",
          	                    	data:{
    		...
    		function findlistpost(sum,str) {
    			...
    			init1("HfindFile",str);
    	   	       $.ajax({
       	        	url:"${pageContext.request.contextPath}/HfindFile",
       	        	dataType:"JSON",
       	        	data:{
       	        		postId:str
       	        	},
       	        	...
       	        	success:function(data){
       	        		...
       	        		var str='';
       	        		var str1='';
       	        		str1=str1+'<tr><th><input type="checkbox" lay-filter="checkbox_v1" name="" ></th> <th>ID</th><th> 标题</th><th>分类</th><th>来源</th><th>更新时间</th><th>下载次数</th><th>操作</th></tr>';
       	        		for (var i = pagecount; i < pagecount1; i++) {
       	        			str=str+'<tr><td><input type="checkbox" lay-filter="checkbox1" class="checktt"  value="'+((data.list)[0])[i].fileId+'" name=""></td><td>'+
    						((data.list)[0])[i].fileId+'</td><td><u style="cursor:pointer" onclick="question_show()">'+
    						((data.list)[0])[i].fileName+'</u></td><td >'+
    						((data.list)[1])[i]+'</td><td >'+
    						((data.list)[0])[i].fileUpAdmin+'</td><td >'+
    						timestampToTime(((data.list)[0])[i].fileCreateTime)+'</td><td >'+
    						((data.list)[0])[i].fileLoadCount+'</td><td class="td-manage">';
    		            	if ($("#hiddenvalue").val() != "true") {
    		            		$(".td-manage").find("a").attr("onclick","warnFunction()");
    							...
    							str=str+'<a title="编辑" href="javascript:;" onclick="question_edit(\'编辑\',\'X-admin/wenjian-edit.jsp?id='+((data.list)[0])[i].fileId+'\',\'4\',\'\',\'510\')" class="ml-5" style="text-decoration:none">'
        						+' <i class="layui-icon">&#xe642;</i>'
        						+'</a><a title="删除" href="javascript:;" onclick="question_del(this,'+((data.list)[0])[i].fileId+')" style="text-decoration:none"> '
        						+'<i class="layui-icon">&#xe640;</i></a>';
    						}
    		            	str=str+'</td></tr>';
    		...
    		$("#post-search-btn").click(function () {
             ...
             function question_show (argument) {
                layer.msg(argument,{icon:6,time:3000});
             }
             /*添加*/
           ...
           function question_edit (title,url,id,w,h) {
            ...
            function question_del(obj,id){
                layer.confirm('确认要删除吗？',function(index){
                   ...
                   // transportId("HdeleteFile",id);
        			$.ajax({
        				url:"${pageContext.request.contextPath}/HdeleteFile",
        				data:{
        					postId:id
        				},
        				dataType:"JSON",
    	...
    	function initList(sum) {
    	       ...
    	       $.ajax({
    	        	url:"${pageContext.request.contextPath}/HselectFileList",
    	        	dataType:"JSON",
    	        	data:{},
    	        	type:"post",
    	        	success:function(data){
    	        		...
    	        		$("#wenjian-sum").text("共有数据："+data.pageCount+"条");
    	        		var str='';
    	        		var str1='';
    	        		str1=str1+'<tr><th><input type="checkbox" lay-filter="checkbox_v1" name="" ></th> <th>ID</th><th> 标题</th><th>分类</th><th>来源</th><th>更新时间</th><th>下载次数</th><th>操作</th></tr>';
    	        		for (var i = pagecount; i < pagecount1; i++) {
    						str=str+'<tr><td><input type="checkbox" lay-filter="checkbox1" class="checktt"  value="'+((data.list)[0])[i].fileId+'" name=""></td><td>'+
    						((data.list)[0])[i].fileId+'</td><td><u style="cursor:pointer" onmouseover="question_show(\''+((data.list)[0])[i].fileDes+'\')">'+
    						((data.list)[0])[i].fileName+'</u></td><td >'+
    						((data.list)[1])[i]+'</td><td >'+
    						((data.list)[0])[i].fileUpAdmin+'</td><td >'+
    						timestampToTime(((data.list)[0])[i].fileCreateTime)+'</td><td >'+
