...
<html>
    ...
    <body>
        ...
        <div class="x-body">
            ...
            <xblock><button id="postCommond-dellall" class="layui-btn layui-btn-danger" ><i class="layui-icon">&#xe640;</i>批量删除</button><button class="layui-btn" onclick="question_add('添加问题','X-admin/postcommond-add.jsp','400','200')"><i class="layui-icon">&#xe608;</i>添加</button><span class="x-right" id="tuijian-sum" style="line-height:40px">共有数据：88 条</span></xblock>
            <table class="layui-table">
                ...
                <tbody id="post-tbody-list">
                    <tr>
                        ...
                        <td class="td-manage">
                            <a title="编辑" href="javascript:;" onclick="question_edit('编辑','X-admin/question-edit.jsp','4','','510')"
        ...
        <script>
        	...
        	$(document).ready(function () {
        		init1("HselectPostCommendList","0");
        		//alert("1");
        		initList(1);
			});
        	function init1(UU,postId) {
               ...
               ///   layform = layui.form;//表单,'form'
                  InitPage(UU);
                  //批量删除提交
                $("#postCommond-dellall").click(function () {
                	 if (checkboxlist.length == 0) {
       				 ...
       				 layer.confirm('确认要删除吗？',function(index){
       	                    //捉到所有被选中的，发异步进行删除
       	                    $.ajax({
       	                    	url:"${pageContext.request.contextPath}/HdeletePostCommondList",
       	                    	dataType:"JSON",
       	                    	type:"post",
       	                    	data:{
    		...
    		function findlistpost(sum,str) {
    			//InitPage("HfindPostCommond");
    			init1("HfindPostCommond",str);
    	   	       $.ajax({
       	        	url:"${pageContext.request.contextPath}/HfindPostCommond",
       	        	dataType:"JSON",
       	        	data:{
       	        		postId:str
       	        	},
       	        	...
       	        	success:function(data){
       	        		...
       	        		var str='';
       	        		var str1='';
       	        		str1=str1+'<tr><th><input type="checkbox" lay-filter="checkbox_v1" name="" ></th> <th>ID</th><th> 标题</th><th>来源</th><th>更新时间</th><th>浏览次数</th><th>操作</th></tr>';
       	        		for (var i = pagecount; i < pagecount1; i++) {
       						str=str+'<tr><td><input type="checkbox" lay-filter="checkbox1"  value="'+((data.list))[i].postId+'" name=""></td><td>'+
       						((data.list))[i].postId+'</td><td><u style="cursor:pointer" onclick="question_show()"><a target="_blank" href="${pageContext.request.contextPath}/togoPostDetails?id='+((data.list))[i].postId+'">'+
    						((data.list))[i].postTitle+'</a></u></td>'+'<td >'+
       						((data.list))[i].postAdmin+'</td><td >'+
       						timestampToTime(((data.list))[i].postUpdateTime)+'</td><td >'+
       						((data.list))[i].postReward+'</td><td class="td-manage">'
       						+''
       						+' '
       						+'<a title="删除" href="javascript:;" onclick="question_del(this,'+((data.list))[i].postId+')" style="text-decoration:none"> '
    		...
    		$("#post-search-btn").click(function () {
    			...
    			if ((s1 != '' && s2 != '') || s3 != '' ) {
    				...
    				//alert(str);
    				strtt=str;
    				//transportId("HfindPostCommond",str);	
    				findlistpost(1,str) 
    			}
    		});            
           ...
           function question_edit (title,url,id,w,h) {
            ...
            function question_del(obj,id){
                layer.confirm('确认要删除吗？',function(index){
                    ...
                    transportId("HdeletePostCommond",id);
    	...
    	function initList(sum) {
    	       ...
    	       $.ajax({
    	        	url:"${pageContext.request.contextPath}/HselectPostCommendList",
    	        	dataType:"JSON",
    	        	data:{},
    	        	type:"post",
    	        	success:function(data){
    	        		...
    	        		var str='';
    	        		var str1='';
    	        		$("#tuijian-sum").text("共有数据："+data.pageCount+"条");
    	        		str1=str1+'<tr><th><input type="checkbox"  name="" ></th> <th>ID</th><th> 标题</th><th>来源</th><th>更新时间</th><th>浏览次数</th><th>操作</th></tr>';
    	        		for (var i = pagecount; i < pagecount1; i++) {
    						str=str+'<tr><td><input type="checkbox"  class="checktt"  value="'+((data.list))[i].postId+'" name=""></td><td>'+
    						((data.list))[i].postId+'</td><td><u style="cursor:pointer" onclick="question_show()"><a target="_blank" href="${pageContext.request.contextPath}/togoPostDetails?id='+((data.list))[i].postId+'">'+
    						((data.list))[i].postTitle+'</a></u></td>'+
    						'<td >'+
    						((data.list))[i].postAdmin+'</td><td >'+
    						timestampToTime(((data.list))[i].postUpdateTime)+'</td><td >'+
    						((data.list))[i].postReward+'</td><td class="td-manage">'
